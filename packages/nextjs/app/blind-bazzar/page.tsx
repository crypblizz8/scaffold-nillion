...
const Home: NextPage = () => {
  ...
  const [programName] = useState<string>("blind_bidding");
  ...
  const [parties] = useState<string[]>(["Seller", "Bidder1", "Bidder2"]);
  const [outputs] = useState<string[]>(["is_bidder1_winner", "seller_price"]);
  ...
  async function handleSecretFormSubmit(
    ...
    if (programId) {
      let partyName;
      if (secretName === "seller_price") {
        partyName = parties[0];
      } else if (secretName === "bid1") {
        partyName = parties[1];
      } else {
        partyName = parties[2];
      }
